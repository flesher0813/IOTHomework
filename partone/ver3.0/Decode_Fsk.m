...
%    pre_for1=fsk_decode(position(index):position(index)+symbol_length-1).*sig_1;
    ...
    if peak>symbol_length/6 && fx(pos)<f_high+500 && fx(pos)>f_high-500
        symbol(index)=1;
    elseif peak>symbol_length/6 && fx(pos)<f_low+500 && fx(pos)>f_low-500
        symbol(index)=0;
    else
        symbol(index)=0;
    end
...
% flag_for0=sum(pre_for0);
% flag_for1=sum(pre_for1);
if peak>symbol_length/6 && fx(pos)<7000 &&fx(pos)>5000
    symbol(length(position))=1;
elseif peak>symbol_length/6 && fx(pos)<5000 &&fx(pos)>3000
    symbol(index)=0;
else
    symbol(index)=0;
end
