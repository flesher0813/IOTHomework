...
%设计高低频的时候考虑相位连续
f_low=4000;     %FSK的低频部分
f_high=6000;    %FSK的高频部分

symbol_length=500;      %一个中文字符有16的bit加一个校验位，这样相当于1秒大概30个字符
...
%这部分用于最开始的定位
channel=chirp(chirp_temp,1000,4*symbol_length/fs,2000);
...
for i=1:head_length
    ...
    sig_temp=[preamble,header_code,data_sig,sig_chirp]; %epilog,sig_interval];      %这是一段的编码
    bit_data_temp=[preamble_order,head_bit,bit_string',0]; %epilog_order,epilog_order,0];
    bit_datas=[bit_datas,bit_data_temp];
    sig=[sig,sig_temp];
end


sig=[sig_interval,sig_interval,sig_interval,sig_interval,sig_interval,sig_interval,sig_interval,sig_interval,channel,sig,sig];     %先象征性的重复两遍
bit_datas=[bit_datas,bit_datas];
